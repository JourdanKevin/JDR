<?php
    function VERIFICATION($tvalue){
        return "SELECT user.id, user.nom, user.prenom, user.mdp, user.mail, typeuser.nomType, typeuser.mj, typeuser.joueur,typeuser.admin, user.pseudo FROM user JOIN typeuser WHERE pseudo=\"".$tvalue['user']."\" AND mdp=\"".$tvalue['mdp']."\"AND user.id_typeUser=typeuser.id;";
    }
